...
- **G3 行为沙箱验证**：在隔离容器中运行技能，进行网络隔离、文件系统限制、资源监控以及预配置依赖环境。⚠️ Phase 3 — 当前版本未实现。
- **G4 权限清单校验**：对照技能声明的 `allowed-tools` 与实际行为进行比对，确保无越权。⚠️ Phase 3 — 当前版本未实现。
...
| 角色           | 职责                                               | 执行主体                  |
| -------------- | -------------------------------------------------- | ------------------------- |
| **Structurer** | 仓库克隆、AST 解析、依赖图构建、机械式四元组预标注 | 确定性脚本 `structure.py` |
| **Extractor**  | 技能筛选、边界合并、评分精炼、命名、上下文补充     | Agent（利用推理能力）     |
| **Reviewer**   | 语义保真审查、幻觉检测、G1–G2 安全扫描、质量评级   | Agent + 脚本混合          |

三个角色在 Agent 的协调下依次执行，通过清晰的输入/输出 JSON 进行交接。

**Python vs Agent 边界原则**：Python 脚本只做机械式工作——解析 AST、构建依赖图、填充函数签名和 docstring。**所有涉及质量判断的决策——哪些函数值得成为技能、如何合并、如何命名、如何评分——全部由 Agent 执行。** Structurer 不解码函数语义、不过滤候选、不推断 `allowed-tools`。它产出的 `analysis.json` 包含所有函数的原始数据，Agent 从中自主筛选。

规则版 `extractor.py` 提供基线评分作为**提示（hint）**，Agent 可参考但不受其约束。Agent 应在 `analysis.json` 的全量函数列表中独立筛选，而非仅从 `candidates.json` 的 Top 5 中选择。
...
Token 估算采用 `字符数 / 4` 的启发式规则（英语文本平均 ~4 字符/token）。
...
- 步骤 6：使用预定义通用测试脚本或通过 `--skip-g3` 跳过。⚠️ `--skip-g3` 在当前版本中为占位标志（G3 属 Phase 3 范围）。
...
| AST 解析    | `tree-sitter` + 语法包  | 支持 Python/JS/TS/Go/Rust ⚠️ Phase 4 — 当前使用 Python stdlib `ast`                      |
